...
p1 <- mpg %>% 
  filter(class %in% c("compact", "midsize", "subcompact", "suv")) %>% 
  ggplot(aes(x = drv, y = hwy, fill = class)) +
  geom_boxplot() +
  theme_minimal() 
...
p1 <- mpg %>%
  ...
  theme_minimal() +
  labs(
    title = "Wydajność paliwowa na autostradzie",
    subtitle = "Podział na klasę auta i rodzaj napędu (f = przód, r = tył, 4 = 4x4)",
    x = "Rodzaj napędu",
    y = "Spalanie na autostradzie (mpg)"
  )
...
plot_ly(
  ...
  hovertemplate = paste('<b>%{text}</b><br><b>X</b>: %{x}<br><b>Y</b>: %{y}')
) %>% 
  layout(
    legend = list(
      x = 0.1, y = 0.9, 
...
plot_ly(df, x = ~HP, type = "histogram", nbinsx = 30)
...
labels <- c("HP", "Attack", "Defense", "Sp..Atk", "Sp..Def", "Speed")
labels_closed <- c(labels, labels[1])
pikachu <- unname(df[df$Name == "Pikachu", labels])
pikachu_closed <- c(pikachu, pikachu[1])

hover_text <- paste0("<b>Statystyka:</b> ", labels_closed, "<br>",
                     "<b>Wartość:</b> ", pikachu_closed)

plot_ly(type = 'scatterpolar', mode = 'markers+lines') %>% 
  add_trace(
    r = pikachu_closed,
    theta = labels_closed,
    fill = "toself",
  ...
  layout(
    polar = list(
      radialaxis = list(
        visible = TRUE,
        range = c(0, 90) # Maksymalna statystyka Pikachu to 90
      )
    ),
    title = "Porównanie statystyk: Pikachu",
    showlegend = FALSE
  )
